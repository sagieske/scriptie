...
class Main(object):
	...
	def create_traintestdata(self, array, posbow, negbow, indexset,mode, **kwargs):
		...
		for tweetindex in indexset:
			...
			if ('tryout' in mode):
				totalbow = dict(posbow.items() + negbow.items())
				vec =  self.tweet_to_vector_numbers(tweet, totalbow)
	...
	def tweet_to_vector_numbers(self, tweet_token, totalbow):
		tweetstring = ' '.join(tweet_token)		# for comparison value tuples in string
		vec = []
		for index,x in enumerate(totalbow):
			if (' '.join(x) in tweetstring):
	...
	def write_results_to_file(self, results):
		...
		f = open('results12juni.txt', 'a')
		f.write(string)
		f.close()

	def write_begin(self):
		string = 'MODE'.ljust(15) + 'GAMMA'.ljust(9) + 'C'.ljust(5) + 'NGRAM'.ljust(10) + 'BOW'.ljust(10)+ 'F1'.ljust(10) +'F1_0'.ljust(10)+'F1_1'.ljust(10) + 'Precision'.ljust(10)+ 'P_0'.ljust(10) + 'P_1'.ljust(10) + 'Recall'.ljust(10) + 'R_0'.ljust(10) + 'R_1'.ljust(10) + '\n'
		f = open('results12juni.txt', 'w+')
		f.write(string)
		f.close()
...
m.write_begin()
"""
modes = ['lemma posneg', 'lemma pos1']
ngramarray = [[1], [2], [1,2], [1,2,3], [2,3]]
lenbows = [10,20,25,30,40,50]
"""
modes = ['lemma tryout']
ngramarray = [[1],[1,2], [1,2,3], [2,3]]
lenbows = [10, 20, 30, 40, 50,60, 70, 80]
...
#m.run('stem posneg', [1,2], 0,0, 40)
print "TIME TAKEN: %f seconds" % (time.time() - begin)

tuples1, tuples2, tuples3 = m.compare_dummy()
resulttuple1 = [('dummy freq', 0.0, 0.0, [1,2], 40, tuples1)]
m.write_results_to_file(resulttuple1)
resulttuple2 = [('dummy strat', 0.0, 0.0, [1,2], 40, tuples2)]
m.write_results_to_file(resulttuple2)
resulttuple3 = [('dummy unif', 0.0, 0.0, [1,2], 40, tuples3)]
m.write_results_to_file(resulttuple3)
